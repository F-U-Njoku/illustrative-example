...
class FeatureSelection(BinaryProblem):
    ...
    def __init__(self, x: pd.DataFrame, y: pd.Series, pop_size: int, classifier):
        ...
        self.obj_labels = ["mRMR", "CMIM", "JMI", "MIM"]
        self.count = 0
    ...
    def evaluate(self, solution: BinarySolution) -> BinarySolution:
        ...
        # JMI
        idx, jmi_score, _ = JMI.jmi(x_train.values, self.target.values, n_selected_features=len(idx))
        # mim
        idx, mim_score, _ = MIFS.mifs(x_train.values, self.target.values, n_selected_features=len(idx))
        ...
        solution.objectives[2] = np.average(jmi_score) * -1
        solution.objectives[3] = np.average(mim_score) * -1
