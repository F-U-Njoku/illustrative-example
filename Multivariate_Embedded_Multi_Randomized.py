...
class FeatureSelection(BinaryProblem):
    ...
    def evaluate(self, solution: BinarySolution) -> BinarySolution:
        ...
        X_train, X_test, y_train, y_test = train_test_split(self.F[idx], self.target,
...
problem = FeatureSelection(X_wrapper, y, population_size, lr)
...
for i in range(len(solutions)):
    ...
    for index, bits in enumerate(list(map(int, solutions[i].get_binary_string()))):
        if bits:
            subset.append(X_wrapper.columns[index])
